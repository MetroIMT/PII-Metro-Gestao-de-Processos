...
router.post("/", auth(), async (req, res) => {
  ...
  if (!["instrumento", "ferramenta"].includes(itemTipo)) {
    return res.status(400).json({ error: 'itemTipo precisa ser "instrumento" ou "ferramenta"' });
  }
  ...
  const collectionName = itemTipo === "instrumento" ? "instrumentos" : "ferramentas";
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const inc =
        tipo === "entrada" || tipo === "devolucao" ? quantidadeNum : -quantidadeNum;
      const status =
